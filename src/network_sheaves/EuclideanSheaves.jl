...
R^n -> R^m represented by a matrix.
"""
@auto_hash_equals struct EuclideanSheaf <: AbstractNetworkSheaf
    vertex_stalks::Vector{Int}
    edge_stalks::Dict{UnorderedPair{Int},Int}
    underlying_graph::Graph
    restriction_maps::Dict{Pair{Int},Matrix{Float64}}
end

EuclideanSheaf(vertex_stalks::Vector{Int}) = EuclideanSheaf(vertex_stalks, Dict{UnorderedPair{Int},Int}(), Graph(length(vertex_stalks)), Dict{Pair{Int},Matrix{Float64}}())
...
"""
function add_sheaf_edge!(s::EuclideanSheaf, v1::Int, v2::Int, rm1::Matrix{Float64}, rm2::Matrix{Float64})
...
function coboundary_map(s::EuclideanSheaf)
    I = Int64[]
    J = Int64[]
    V = Matrix{Float64}[]
